...
const GameInfoBoard = ({ info }: Props) => {
    ...
    const teamColors = ['#736372', '#7A918D']; // Light theme team mapping
    const teamNames = ['BLUE', 'RED'];

    return (
        <Paper sx={{ p: 2, height: '100%', bgcolor: 'background.paper', overflowY: 'auto' }}>
            <Grid container alignItems="center" spacing={2}>
                ...
                {info.teams && info.teams.map((team, idx) => (
                    <Grid item xs={4} key={idx} sx={{ textAlign: 'center', borderLeft: idx === 1 ? '1px solid #DDDDDD' : 'none' }}>
                        ...
                        {team.totalPenaltyCount !== undefined && (
                            <Typography variant="body2" sx={{ mt: 1, color: '#736372', fontWeight: 'bold' }}>
                        ...
                        {/* Coach Message - 삭제 */}
                        {team.coachMessage && (
                            <Typography variant="caption" display="block" sx={{ mt: 0.5, fontStyle: 'italic', color: 'text.secondary', border: '1px dashed #AAAAAA', borderRadius: 1, p: 0.5 }}>
