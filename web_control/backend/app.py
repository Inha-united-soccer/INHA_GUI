...
# -----------------------------------------------------------------------------------------
# Dynamic Role / Parameter Updates
# -----------------------------------------------------------------------------------------
...
class StrategyParamApply(BaseModel):
    robot_ids: List[str]
    strategy_name: str
    params: Dict[str, Any]

@app.post("/api/set_role")
def set_role_endpoint(data: RoleSwitch):
    print(f"[API] Set Role request to {data.robot_id}: {data.role_name}")
    ...
    for robot_id in targets:
        if robot_id not in ssh_manager.clients: continue
        success, msg = ssh_manager.set_role(robot_id, data.role_name)
        if success:
            success_count += 1
            print(f"[Role] Success to {robot_id}")
        else:
            print(f"[Role] Failed to {robot_id}: {msg}")

    if success_count > 0:
        return {"status": "success", "message": f"Role set on {success_count} robots"}
    else:
        raise HTTPException(status_code=500, detail="Failed to set role on any robot")
...
@app.post("/api/update_parameter")
def update_parameter_endpoint(data: ParamUpdate):
    print(f"[API] Update Parameter request to {data.robot_id}: {data.param_name} = {data.param_value}")
    ...
    for robot_id in targets:
        if robot_id not in ssh_manager.clients: continue
        success, msg = ssh_manager.update_parameter(robot_id, data.param_name, data.param_value)
        if success:
            success_count += 1
            print(f"[Param] Success to {robot_id}")
        else:
            print(f"[Param] Failed to {robot_id}: {msg}")

    if success_count > 0:
        return {"status": "success", "message": f"Parameter updated on {success_count} robots"}
    else:
        raise HTTPException(status_code=500, detail="Failed to update parameter on any robot")
...
async def list_strategies():
    # Search in multiple possible locations for subtrees
    search_dirs = [
        STRATEGY_DIR,
        os.path.join(os.path.dirname(__file__), "../../INHA_Soccer/INHA-Player/src/brain/behavior_trees/subtrees"),
        "/Users/mingyu/Workspace/INHA_GUI/INHA_Soccer/INHA-Player/src/brain/behavior_trees/subtrees"
    ]
    
    all_files = []
    for d in search_dirs:
        if os.path.exists(d):
            all_files.extend(glob(os.path.join(d, "*.xml")))
            
    unique_names = sorted(list(set([os.path.basename(f) for f in all_files])))
    return {"strategies": unique_names}

@app.get("/api/strategy_params/{name}")
async def get_strategy_params(name: str):
    params, error = ssh_manager.get_strategy_params(name)
    if error:
        raise HTTPException(status_code=404, detail=error)
    return {"params": params}

@app.post("/api/strategy_params/apply")
async def apply_strategy_params(data: StrategyParamApply):
    results = []
    for robot_id in data.robot_ids:
        success, msg = ssh_manager.apply_strategy_params(robot_id, data.strategy_name, data.params)
        results.append({"robot_id": robot_id, "success": success, "message": msg})
